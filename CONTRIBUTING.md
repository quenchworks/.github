...
Thanks for helping build hardened, verifiable infrastructure images that stay free to use.
...
- **Report a bug:** a chart that won't render or install, an image that won't start, or a failing scan.
- **Improve a chart or build:** PRs are welcome on `images`, `charts`, and `common`.
- **Report a vulnerability:** see [SECURITY.md](./SECURITY.md). Do not open a public issue.

## The one hard rule: clean room

Quenchworks is independent. Charts must be written from each application's own upstream
documentation, never copied or adapted from another vendor's charts, including their structure,
values schemas, or image layout. PRs that copy third-party chart implementations will be declined.
...
3. Open a PR. CI runs the gates above, and green CI is required to merge.
